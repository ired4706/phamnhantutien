...
class ItemLoader {
  ...
  async loadAllItems() {
    try {
      ...
      this.loaded = true;
      Logger.info('Items loaded', { items: Object.keys(this.items).length, categories: Object.keys(this.categories).length });
      return this.items;
    } catch (error) {
      ErrorHandler.handle(error, 'ItemLoader.loadAllItems');
      return {};
    }
  }
